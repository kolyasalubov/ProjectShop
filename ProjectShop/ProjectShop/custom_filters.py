...
class MultipleChoiceListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        raise NotImplementedError(
            ...
            "or choices field of some model."
        )
    ...
    def choices(self, changelist):
        ...
        for lookup, title in self.lookup_choices:
            yield {
                ...
                "include_query_string": self.amend_query_string(
                    changelist, include=str(lookup)
                ),
                "exclude_query_string": self.amend_query_string(
                    changelist, exclude=str(lookup)
                ),
                "display": title,
            }
